...
(defun pnpm-menu--parse-arguments (&optional key-prefix)
  "Parse command-line arguments into structured groups and commands.

Optional argument KEY-PREFIX is a string used as a prefix for generated keys."
  (let ((group-title-regex "^\\([a-z][^\n]+\\):$")
        (command-regex
         "^[\s]*\\(\\(-[a-z]\\),[\s]\\)?\\(--\\(\\[?\\([a-z-]+-\\)\\]\\)?[a-z-]+\\)")
        (groups))
    (let ((case-fold-search t))
      (while (and (not (looking-at group-title-regex))
                  (zerop (forward-line 1))))
      (while (looking-at group-title-regex)
        (when-let ((group-title (match-string-no-properties 1)))
          (forward-line 1)
          (let ((commands))
            (while (looking-at command-regex)
              (let ((short (match-string-no-properties 2))
                    (long  (match-string-no-properties 3))
                    (no-option (match-string-no-properties 4))
                ...
                (when (looking-at "[\s]+\\(<\\([a-z][^>]+\\)>\\)")
                  (setq specifier (match-string-no-properties 1))
                  (goto-char (match-end 0))
                  (skip-chars-forward "\s")
                ...
                (while (and (not (or
                                  (looking-at group-title-regex)
                                  (looking-at command-regex)))
                            (zerop (forward-line 1)))
                  (setq descr-end (line-end-position)))
                (setq descr (string-join
                             (split-string
                              (buffer-substring-no-properties descr-start descr-end) nil t)
                             " "))
                (push (list short long descr specifier) commands)))
            (push (cons group-title (nreverse commands)) groups))))
      (setq groups (nreverse groups))
      (let ((results)
            (used-keys '("q")))
                      ...
                      (pcase-let*
                        ...
                        (pcase specifier
                          ...
                          (_ (list key
                                   ...
                                   arg)))))
                    used-keys)))
              (push (apply #'vector (append (list group-name) items)) results))))
        (apply #'vector (nreverse results))))))

(defun pnpm-menu--eval-command-arguments (cmd &optional pnpm-version)
  "Evaluate and define a transient prefix command for CMD with OPTIONS.

Argument CMD is the command to be evaluated.

Optional argument PNPM-VERSION is the version of pnpm to use."
  (let ((options (npmjs-parse-help-with-output
                     (shell-command-to-string (concat "pnpm help "
                                                      cmd))
                   (pnpm-menu--parse-arguments
                    (pcase cmd
                      ("run" "-")))))
        (name
         (make-symbol
    ...
    (pcase cmd
      ("run" (eval `(progn
                      ...
                      ',name)
                   t))
      (_ (eval `(progn (transient-define-prefix ,name ()
                         ,options
                         [[("RET" "Run" (lambda ()
                                          (interactive)
                                          (npmjs-run-compile
          ...
          (let ((commands)
            ...
            (while (looking-at command-regex)
              (let ((short (match-string-no-properties 3))
                ...
                (push (list short long descr) commands)))
            (push (cons group-title (nreverse commands)) groups))))
      (setq groups (nreverse groups)))))

(defun pnpm-menu--version ()
  "Return the trimmed version string of the installed pnpm."
  (string-trim (shell-command-to-string "pnpm --version")))
